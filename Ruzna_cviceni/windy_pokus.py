...
def points_read():
    work_book = xlrd.open_workbook('./点位表.xlsx')
...
def forecast_point(name, lat, lon, alt, days):
    post = {
        ...
        "lon": lon,
        "model": "gfs",
        "parameters": ["temp", "wind", "precip", "rh", "pressure", "ptype", "lclouds", "mclouds", "hclouds",
                       "windGust"],
        "levels": ["surface"],
    ...
    index = []
    for i in resault['ts']:
        if time.strftime("%Y-%m-%d", time.localtime(int(str(i)[:10]))) == time.strftime("%Y-%m-%d",
    ...
    wind_dir = 0
    for i in index:
        hour.append(time.strftime("%Y%m%d%H", time.localtime(int(str(resault['ts'][i])[:10]))))
    ...
    if ptype == 1:
        ...
        if 25 <= sum(forecast[5]) < 50:
            weather = 'dešťová bouře'
        if 50 <= sum(forecast[5]):
            weather = 'bouře'

    if ptype == 7:
        weather = 'směs sněhu a deště'

    if ptype == 0:
        if 0.2 <= mean(lcloud + mcloud + hcloud) < 0.6:
            weather = 'oblačno (meteorologie)'
        if mean(lcloud + mcloud + hcloud) >= 0.6:
            weather = '阴'
    else:
        weather = 'něco'

    forecast_resault = [weather, temp_resault, rh_resault, pressure_resault, wind_number_resault, wind_dir,
                        gust_resault, precip_resault]
...
print(forecast_point("Ostrava", 49.50, 18.17, 200, 1))